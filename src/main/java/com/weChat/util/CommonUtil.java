...
public class CommonUtil {
    ...
    public static String getIP(String accessToken){
        String IP = "";
        String requestUrl = Urls.ip_url.replace("ACCESS_TOKEN",accessToken);
        // 发起GET请求获取凭证
        JSONObject jsonObject = httpsRequest(requestUrl, "GET", null);
        if(null != jsonObject){
            try {
                ...
                log.error("获取IP失败："+ ExceptionUtil.getErrorInfo(e));
            }
        }
        return IP;
    }
...
//        String requestUrl = Urls.token_url.replace("APPID", appid).replace("APPSECRET", appsecret);
//        try {
//            com.alibaba.fastjson.JSONObject jsonObject = JSON.parseObject(HttpClientUtil.executeHttpGet(requestUrl));
//            token = new Token();
//            token.setAccessToken(jsonObject.getString("access_token"));
//            token.setExpiresIn(Integer.parseInt(jsonObject.getString("expires_in")));
//        }catch (Exception e){
//            log.info("获取token错误："+ ExceptionUtil.getErrorInfo(e));
//        }
//        return token;
//    };
}
